...
async def main(port: Optional[int]) -> None:
    ...
    cur_class_hash = declarations["pragma_Randomness"]
    logger.info(f"ℹ️  Current implementation hash: {hex(cur_class_hash)}")

    new_implementation_hash = await declare_v2("pragma_Randomness", port=port)
    logger.info(f"ℹ️  New implementation hash: {hex(new_implementation_hash)}")

    tx_hash = await invoke(
        "pragma_Randomness", "upgrade", [new_implementation_hash], port=port
    )

    declarations["pragma_Randomness"] = new_implementation_hash
