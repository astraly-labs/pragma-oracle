...
mod PublisherRegistry {
    ...
    impl PublisherRegistryImpl of IPublisherRegistry<ContractState> {
        ...
        fn add_sources_for_publisher(ref self: ContractState,publisher: felt252, sources: Span<felt252>) {
            let mut idx: u32 = 0;
        ...
        fn add_publisher(ref self: ContractState, publisher: felt252, publisher_address: ContractAddress) {
            let state: Admin::ContractState = Admin::unsafe_new_contract_state();
        ...
        fn update_publisher_address(ref self : ContractState, publisher: felt252, new_publisher_address: ContractAddress) {
            let state: Admin::ContractState = Admin::unsafe_new_contract_state();
            Admin::assert_only_admin(@state);
        ...
        fn _build_array(self : @ContractState, index: usize, len: usize, ref publishers: Array<felt252>) {
            if index >= len {
                return ();
            }
